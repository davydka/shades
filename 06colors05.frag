...
void main() {
    ...
    float angle = easing(atan(toCenter.y, toCenter.x), 0.2);
    float radius = length(toCenter)*2.0;

    float anim = abs(fract(u_time/10.0));
    // anim = 0.5;
    // color = hsb2rgb(vec3((easing(angle, 0.1)/TWO_PI)+anim, circularEaseIn(radius)*4.0, 1.0));
